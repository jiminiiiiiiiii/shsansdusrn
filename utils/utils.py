...
def train_encdec(model, criterion, optimizer, train_dl, scaler):
    ...
    for x, y, tgt_mask in train_dl:
        ...
        x_input.append(scaler.inverse_transform(np.reshape(
            np.array(x[0].view(-1).detach().numpy()), (x.shape[1], 1))))
        truth.append(scaler.inverse_transform(np.reshape(
            np.array(y[0].view(-1).detach().numpy()), (y.shape[1], 1))))
        predicted.append(scaler.inverse_transform(np.reshape(
            np.array(output[0].detach().view(-1).numpy()), (output.shape[1], 1))))
...
def test_model(test_dl, model, scaler):
    ...
    with torch.no_grad():
        ...
        for x, y, mask in test_dl:
            ...
            x_input.append(scaler.inverse_transform(np.reshape(
                np.array(x[0].view(-1).numpy()), (x.shape[1], 1))))  # (x.shape[1],1))))
            truth.append(scaler.inverse_transform(np.reshape(
                np.array(y[0].view(-1).numpy()), (y.shape[1], 1))))
            predicted.append(scaler.inverse_transform(np.reshape(
                np.array(output[0].view(-1).numpy()), (output.shape[1], 1))))
            # scaler for better visualization
    return x_input, truth, predicted


def test_enc_dec_model_seq_at_a_time(
    test_dl, model, scaler, output_sequence_length):
    """Return input, truth and prediction for input model and data"""
    x_input = []
    truth = []
    predicted = []
    ...
    with torch.no_grad():
        ...
        for x, y, mask in test_dl:
            ...
            # out =
            # model.out(model.decoder(model.positional_encoding(outputs),memory,
            # tgt_mask))
            ...
            for i in range(1, output_sequence_length + 1, 1):
                ...
                dec_in = dec_in.unsqueeze(-1).to(device)

                out = model.out(
    model.decoder(
        model.positional_encoding(dec_in),
        memory,
         tgt_mask))
            ...
            truth.append(scaler.inverse_transform(np.reshape(
                np.array(y[0].view(-1).numpy()), (y.shape[1], 1))))
            predicted.append(scaler.inverse_transform(np.reshape(
                np.array(out[0].view(-1).numpy()), (out.shape[1], 1))))
                # scaler for better visualization
    return x_input, truth, predicted
    ...
    with torch.no_grad():
        ...
        for x, y, mask in test_dl:
            ...
            for i in range(1, output_sequence_length + 1, 1):
                ...
                dec_input = outputs[:i].to(device)  # .unsqueeze(-1)
                out = model.out(
    model.decoder(
        model.positional_encoding(dec_input),
        memory,
         tgt_mask))
...
def plot_output(
    ...
    epochs,
    lr,
    output_for='_test_output_',
     figtext=None):
    # plt.figure(figsize=(30,10))
    # plt.plot(torch.Tensor(x_input).view(-1),'b',linewidth=1)
    plt.plot(np.arange(dataset_params['window_size'], torch.Tensor(truth).view(-1).numpy().shape[0] + int(
        dataset_params['window_size']), 1), torch.Tensor(truth).view(-1), 'g', linewidth=1)
    plt.plot(np.arange(dataset_params['window_size'], torch.Tensor(predicted).view(-1).numpy().shape[0] + int(
        dataset_params['window_size']), 1), torch.Tensor(predicted).view(-1), 'r', linewidth=1)
    plt.grid(axis='x')
    # plt.legend(["${input}$", "${predicted}$", "${truth}$"])
    plt.legend(["${truth}$", "${predicted}$"])
    plt.ylabel("job arrival count {scaled}")  # , fontsize=20)
    plt.xlabel(
    "time steps {sample rate " +
    str(time_interval) +
     " min interval)")  # , fontsize=20)
    if output_for == '_test_output_':
        plt.suptitle(
    str(model_name) +
     " { Test output Google Cluster }")  # , fontsize=20)
    else:
        plt.suptitle(
    str(model_name) +
     " { Train output Google Cluster }")  # , fontsize=20)
    if figtext != None:
...
def mean_absolute_scaled_error(y_true, y_pred):
    ...
    n = y_true.shape[0]
    # Not applicable : differencing is done at axis = 1 beacuse we have a a 3D
    # array [window size, batch_size, features]
...
def predict(model, input_sequence, forecasting_step, input_window):
    ...
    with torch.no_grad():
        for i in range(forecasting_step):
            data = torch.reshape(torch.Tensor(
                forecast_output[-input_window:]), (1, input_window, 1)).to(device)
            output = model(data)
            output = output.cpu()
            # input_sequence = torch.cat((input_sequence,
...
def train_tf_encdec(
    ...
    # initialize losses
    mse_loss, shape_loss, temporal_loss, mae_loss = torch.tensor(
    ...
    for x, y, tgt_mask in train_dl:
        ...
        truth.append(scaler.inverse_transform(np.reshape(
            np.array(y[0].view(-1).numpy()), (y.shape[1], 1))))
        predicted.append(scaler.inverse_transform(np.reshape(
            np.array(outputs[0].view(-1).numpy()), (outputs.shape[1], 1))))
...
def test_tf_enc_dec(test_dl, model, scaler, forecasting_step, device):
    """Return input, truth and prediction for input model and data"""
    x_input = []
    truth = []
    predicted = []
...
def train_tf_enc_dec_gan_XX(model, discriminator,
    ...
    # THIS TRAINING FUNCTION UPDATES GRADIENT FOR GENERATOR & DISCRIMINATOR
    # FOR EVERY BATCH
    ...
    with open(EXP_FOLDER_PATH + '/training_graphs/training_terminal_output_' + str(time_interval) + 'mins_sr.txt', 'w') as fp:
        ...
        for e, epoch in enumerate(range(epochs)):
            ...
            for x, y, tgt_mask in train_dl:
                ...
                valid = torch.autograd.Variable(torch.cuda.FloatTensor(2, 64, 1).fill_(
                    1.0), requires_grad=False)  # update the hardcoded tensor size
                fake = torch.autograd.Variable(torch.cuda.FloatTensor(2, 64, 1).fill_(
                    0.0), requires_grad=False)  # update the hardcoded tensor size

                # >>>> generator processing starts here
                # encode x
                enc_out = model.encoder(
    model.positional_encoding(x)).to(device)
    ...
    model.decoder(
        ...
        enc_out,
         tgt_mask))
                ...
                loss = criterion(outputs, y) + lambda_ * \
                                 adverserial_loss(
    discriminator(outputs), valid)
                loss.backward(retain_graph=True)
                optimizer_G.step()

                # to take mean of total loss
                trainG_loss += (loss.item() * x.shape[0])
                nG += x.shape[0]
    ...
    if e % 5 == 0 or e == (epochs - 1):
                smape_a = smape(truth, predicted)  # 여기만 변경
                print("Epoch: {} => SMAPE:{} ; loss_G: {} ; loss_D: {}".format(e, smape_a, trainG_loss/nG, trainD_loss/nG))
                to_write = "\nEpoch: {} => SMAPE:{} ; loss_G: {} ; loss_D: {}".format(e, smape_a, trainG_loss/nG, trainD_loss/nG)
                fp.write(to_write)
                # plot the graph 
                if e % 100 == 0 or e == (epochs - 1): 
                    plot_output(
                    path=EXP_FOLDER_PATH + 'training_graphs/' + str(time_interval) + '_mins/epoch_' + str(epoch),
                    model_name='SMAPE_' + str(smape_a) + ' ' + str(model.model_name),
                    x_input=x_input, truth=truth, predicted=predicted,
                    dataset_params=dataset_params, model_params=modelG_params,
                    time_interval=time_interval, epochs=epochs, lr=lr, output_for='train_output'
        )


    if e % 5 == 0 or e == (epochs - 1):
                smape_a = np.round(mean_absolute_percentage_error(truth, predicted),2)
...
def smape(y_true, y_pred):
    ...
    smape[nonzero_idx] = diff[nonzero_idx] / denominator[nonzero_idx]
...
def train_tf_enc_dec_gan(model, discriminator,
    ...
    if e % 5 == 0 or e == (epochs - 1):
                smape_a = smape(truth, predicted)
                print("Epoch: {} => SMAPE:{} ; loss_G: {} ; loss_D: {}".format(e, smape_a, trainG_loss/nG, trainD_loss/nD))
                to_write = "\nEpoch: {} => SMAPE:{} ; loss_G: {} ; loss_D: {}".format(e, smape_a, trainG_loss/nG, trainD_loss/nD)
                fp.write(to_write)

                # 필요 시 그래프 시각화도 이 기준으로 열 수 있음
                # if e % 100 == 0 or e == (epochs - 1):
                #     plot_output(
                #         path=EXP_FOLDER_PATH + 'training_graphs/' + str(time_interval) + '_mins/epoch_' + str(e),
                #         model_name='SMAPE_' + str(smape_a) + ' ' + str(model.model_name),
                #         x_input=x_input, truth=truth, predicted=predicted,
                #         dataset_params=dataset_params, model_params=modelG_params,
                #         time_interval=time_interval, epochs=epochs, lr=lr, output_for='train_output'
...
def train_and_evaluate_tf_enc_dec_gan(model, discriminator,
    ...
    print("Train Dataset SMAPE:", smape(truth, predicted))
    ...
    mape_a = np.round(mean_absolute_percentage_error(truth, predicted),2)
    print("Test Dataset SMAPE:", smape(truth, predicted))
