...
def _grad_penalty(real_data, gen_data, D):
        ...
        t = t.expand_as(real_data).to(device)
        ...
        gradients = torch_grad(outputs=prob_interpol, inputs=interpol,
                               grad_outputs=torch.ones(prob_interpol.size(), device=device), create_graph=True, retain_graph=True)[0]
        ...
        return gp_weight * (torch.max(torch.zeros(1,dtype=torch.double).to(device))) , grad_norm__

# Improved Wasserstien GAN 
def improved_gradient_penalty(discriminator, real_data, fake_data, device):
    lambda_ = 0.1
    batch_size = real_data.size()[1] 
  
    alpha = torch.rand(batch_size, 1, device=device)  # 생성 시 device 지정
    alpha = alpha.expand(real_data.size())

    interpolates = alpha * real_data + ((1 - alpha) * fake_data)
    interpolates = interpolates.to(device)  # 동일하게 device 사용
    ...
    gradients = autograd.grad(outputs=disc_interpolates, inputs=interpolates, 
                            grad_outputs=torch.ones(disc_interpolates.size(), device=device),create_graph=True,retain_graph=True)[0]
